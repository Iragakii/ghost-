...
export function createBuck(scene) {
    const buckGroup = new THREE.Group();
    // Position next to cha (cha is at x: -140, y: 19, z: -200)
    buckGroup.position.set(-179, 1 , -220); // To the right of cha
    buckGroup.rotation.y = -Math.PI / 2; // Same rotation as cha
    scene.add(buckGroup);
    ...
    gltfLoader.load('/pinklocation/buck.glb', (gltf) => {
        incrementLoaded();
        ...
        if (gltf.animations && gltf.animations.length > 0) {
            ...
            console.log('Buck available animations:', gltf.animations.map(clip => clip.name));
            ...
            console.warn('No animations found in buck.glb');
        }
        ...
        console.error('Error loading buck.glb:', error);
    });
